<div class="wrapper">
<?php if($this->session->userdata('korisnikst')->id == $profil['id']) { ?>
	<form action="<?php echo site_url("KorisnikSt/menjajProfil/".$profil['id']);?>" name="forma" method="post" id="formStart">
		...
		<div class="leftForm" style="float: left; width: 50%">
			...
			<div class="formWrapper wrap-date">
				<input id="datum" type="date" name="datumOsn" value="<?php echo  $profil['datosnivanja'] ?>" class="polje"/>
		...
		<div class="rightForm" style="float: right; width: 50%">
			...
			<div class="formWrapper">
				<textarea id="area" name="predPro" cols="50" rows="6" class="textarea"><?php echo  $profil['posao'] ?></textarea>
				<label for="area">Predlog projekta</label>
			</div>
			<div class="formWrapper">
				<input id="triznos" type="number" name="iznos" value="<?php echo  $profil['tiznos'] ?>" class="polje"/>
				<label for="triznos" class="row">Trazeni iznos</label>
			</div>
			<div class="formWrapper">
				<input id="svojina" type="text" name="statIntS" value="<?php echo  $profil['statintsvojine'] ?>" class="polje"/>
				<label for="svojina" class="row">Status intelektualne svojine</label>
			</div>
			<div class="formWrapper">
				<input id="pat" type="text" name="patent" value="<?php echo  $profil['podopatentu'] ?>" class="polje"/>
...
<?php } else {
	redirect("$controller/izmeniProfil/".$this->session->userdata('korisnikst')->id);
} ?>
</div>
